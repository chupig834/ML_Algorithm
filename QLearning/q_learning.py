...
class QLearningAgent:
    ...
    def reset(self) -> None:
        ...
        ### YOUR CODE HERE ###
        '''
        Q-table only store the latest learned Q-values
        Q-table's size = num state_space * num action_space
            1. create a standard normal table for the q_table
        '''
        ### END OF YOUR CODE ###

        # qtable = np.array(self.q_table)

        self.q_table = np.random.standard_normal(size=(self.state_space.n, self.action_space.n))
    ...
    def act(self, state: int, exploit: bool = False) -> int:
        ...
        #If it is exploit or random() is greater than explore_rate, we will follow the max reward
    ...
    def observe(self, state: int, action: int, reward: float, next_state: int) -> None:
        ...
        transition_sample = Transition(state=state, action=action, reward = reward, next_state = next_state)
        self.replay_buffer.push(transition_sample)
    ...
    def learn(self) -> dict:
        ...
        if batch.states.size == 0:
            return {
                'td_error': np.array([], dtype=float),
        ...
        '''
        A batch is a collection of transitions
            1. Calculate the max of the batch 
            2. Get the current Q value 
            3. apply the equation to get the updated q value? 
        Q_table has the state by q_vaule 
        ...
        next_max_q = np.max(self.q_table[batch.next_states], axis = 1) #This is a (batch_size,) 1D array

        current_q = self.q_table[batch.states, batch.actions]

        target_value = batch.rewards + self.discount * next_max_q

        td_error = target_value - current_q

        self.q_table[batch.states, batch.actions] = self.q_table[batch.states, batch.actions] + self.lr * td_error
    ...
    def get_policy_arr(self) -> np.ndarray:
        ...
        """
        ### YOUR CODE HERE ###
        #Find the max value's index by row
