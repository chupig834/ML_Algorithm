...
def pca_approx(M, m=100):
    ...
    '''
    1. Each eigenvalue tells you how much variance is captured by that engienvector
    '''
    M_tilde = np.log1p(M)
    mean_vec = np.mean(M_tilde, axis=0)
    Mc = M_tilde - mean_vec 
    ...
    pca_full = PCA(n_components=min(Mc.shape))
    pca_full.fit(Mc)

    frac_var = np.sum(eigenvalues) / np.sum(pca.explained_variance_)
...
def compute_embedding(Mc, V):
    ...
    E = P / np.linalg.norm(P, axis = 1, keepdims=True)
