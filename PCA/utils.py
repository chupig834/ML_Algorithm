...
def find_most_sim_word(word, words, E):
    ...
    similarity[word_index] = -np.inf

    most_sim_index = np.argmax(similarity)
...
def find_info_ev(v, words, k=20):
    ...
    # i) Obtain a set of indices (postions in 'v') which would sort the entries of 'v' in decreasing order of absolute value.
    sorted_index = np.argsort(np.abs(v))[::-1]

    # ii) Using the set of first 'k' indices, get the corresponding words from the list 'words'.
    top_k_index = sorted_index[:k]
    info = [words[i] for i in top_k_index]
...
def get_projections(word_seq, words, E, w):
    ...
    for word in word_seq:
        emb_word = find_embedding(word, words, E)
        
        projection = np.dot(emb_word, w_norm)
...
def find_most_sim_word_w(w, words, E, ids):
    ...
    for idx in ids:
        simlarity[idx] = -np.inf

    best_idx = np.argmax(simlarity)
    most_sim_word = words[best_idx]
...
def find_analog(wd1, wd2, wd3, words, E):
    ...
    # else:
    #     idx1 = words.index(wd1)
    #     idx2 = words.index(wd2)
    #     idx3 = words.index(wd3)
    
    # Force words into a plain Python list (works whether it was a list or ndarray)
    words_list = list(words)

    # Then get all three indices with a comprehension
    idx1, idx2, idx3 = (words_list.index(w) for w in (wd1, wd2, wd3))
    
    exclusion = [idx1, idx2, idx3]

    wd4 = find_most_sim_word_w(w, words, E, exclusion)
